...
std::vector<Eigen::Vector3d> get_wc_positions_in_molframe(MDFrame const& mdframe, int oid, Eigen::Matrix3d const& M)
{
...
/*
		...
		std::cout << "-------------------" << std::endl;
*/
		auto const wc_p = get_wc_positions_in_molframe(mdframe, i, Minv);

		wc_out << "mol_id " << mdframe.mol_id[i] << " , oxygen in labframe " 
			<< " " << ref(0) << " " << ref(1) << " " << ref(2) << std::endl;

		for (int iw = 0; iw<wc_p.size(); iw++) 
