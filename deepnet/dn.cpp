...
Eigen::Matrix3d get_transformation_matrix(MDFrame & mdframe, nbrlist_t const& nbrlist, const int i)
{
  ...
  std::cout << mdframe.x[i] <<  " " << mdframe.y[i] << " " <<  mdframe.z[i] << std::endl;
...
//=========================================================================================================
    ...
    for (const auto nbridx : nbridx_lists)
    {
      for (const int j : nbridx)
      {
        ...
        double r_mol_inv = 1.0/r_mol.norm();
        double r_mol_inv_sq = r_mol_inv*r_mol_inv;

        double rij = r_mol.norm();
        double sfunction = 1.0;

        if (rcs < rij && rij < rc)
        {
          sfunction = (0.5*cos(pi*(rij-rcs)/(rc-rcs)) + 0.5);
        } else if (rc < rij) {
          sfunction = 0.0;
        }

        const auto ft_vec = r_mol*r_mol_inv_sq*sfunction;
        ft_out << mdframe.name[j] << " " << r_mol_inv*sfunction << " " << ft_vec(0) << " " << ft_vec(1) << " " << ft_vec(2) << std::endl;
      }
    }
//=========================================================================================================
	}
